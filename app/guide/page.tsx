  ...
  return (
    ...
    >
      <div className="not-prose space-y-10">
        <section className="grid gap-4 rounded-2xl border border-cyan-200 dark:border-cyan-300/15 bg-cyan-50 dark:bg-cyan-300/[0.045] p-5 sm:grid-cols-[1fr_auto] sm:items-center sm:p-6">
          <div>
        ...
        <section aria-labelledby="guide-steps-title">
          <div className="mb-5">
            ...
            <h2 id="guide-steps-title" className="mt-2 text-2xl font-bold text-slate-950 dark:text-white sm:text-3xl">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {steps.map((step) => (
              <article
                key={step.number}
                className="group rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.035] p-5 transition hover:border-cyan-300/25 hover:bg-slate-100 dark:hover:bg-white/[0.055] sm:p-6"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-base font-bold text-slate-950 dark:text-white sm:text-lg">{step.title}</h3>
                    <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{step.description}</p>
                    <p className="mt-3 border-l border-slate-200 dark:border-white/10 pl-3 text-xs leading-5 text-slate-500 dark:text-slate-400">
                      {step.detail}
                    </p>
                  </div>
                </div>
        ...
        <section className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
          <div className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.03] p-5 sm:p-6">
            <span className="text-[11px] font-bold uppercase tracking-[.18em] text-emerald-700 dark:text-emerald-300">
              Reading the dashboard
            </span>
            <h2 className="mt-2 text-2xl font-bold text-slate-950 dark:text-white">What each result means</h2>
            <div className="mt-5 divide-y divide-slate-200 dark:divide-white/10">
              {resultItems.map(([title, description]) => (
                <div key={title} className="grid gap-1 py-4 sm:grid-cols-[150px_1fr] sm:gap-5">
                  ...
                  <span className="text-sm leading-6 text-slate-600 dark:text-slate-400">{description}</span>
                </div>
              ))}
            </div>
          </div>

          <div className="rounded-2xl border border-violet-200 dark:border-violet-300/15 bg-violet-50 dark:bg-violet-300/[0.045] p-5 sm:p-6">
            <span className="text-[11px] font-bold uppercase tracking-[.18em] text-violet-700 dark:text-violet-300">
              Reward slots
            </span>
            <h2 className="mt-2 text-2xl font-bold text-slate-950 dark:text-white">Points and availability are different</h2>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              <Link
                href="/swag-drops/"
                className="inline-flex min-h-10 items-center rounded-lg border border-slate-200 dark:border-white/10 px-4 text-sm font-semibold text-slate-600 dark:text-slate-300 transition hover:bg-slate-100 dark:hover:bg-white/5"
              >
                Reward archive
              </Link>
            </div>
        ...
        <section aria-labelledby="guide-errors-title">
          <div className="mb-5">
            ...
            <h2 id="guide-errors-title" className="mt-2 text-2xl font-bold text-slate-950 dark:text-white sm:text-3xl">
              Common errors
            </h2>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {errors.map((error) => (
              <article key={error.title} className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/10 p-5">
                <h3 className="text-base font-bold text-slate-950 dark:text-white">{error.title}</h3>
                <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">{error.body}</p>
              </article>
            ))}
          </div>
        </section>

        <section className="overflow-hidden rounded-2xl border border-emerald-200 dark:border-emerald-300/15 bg-gradient-to-br from-emerald-50 dark:from-emerald-300/[0.06] via-white dark:via-white/[0.025] to-cyan-50 dark:to-cyan-300/[0.05] p-5 sm:p-7">
          <span className="text-[11px] font-bold uppercase tracking-[.18em] text-emerald-700 dark:text-emerald-300">
            Accuracy and official results
          </span>
          <div className="mt-2 grid gap-5 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              <h2 className="text-2xl font-bold text-slate-950 dark:text-white">Use Arcade Points as a transparent community estimate</h2>
