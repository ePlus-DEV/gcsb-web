...
export function generateStaticParams() {
  return WEBSITE_LOCALES.filter((locale) => locale.path).map((locale) => ({ locale: locale.path }))
}

function resolveLocale(segment: string): WebsiteLocale | null {
  return WEBSITE_LOCALES.find((locale) => locale.path.toLowerCase() === segment.toLowerCase())?.code ?? null
}

const readCatalog = cache(async function readCatalog(locale: WebsiteLocale): Promise<WebsiteCatalog> {
...
function getLocalizedSeo(catalog: WebsiteCatalog) {
  const fallbackTitle = `${catalog.messages.heroTitleTop ?? "CHECK YOUR"} ${catalog.messages.heroTitleBottom ?? "ARCADE SCORE"} – Google Cloud Arcade 2026`
  const title = catalog.messages.pageTitle ?? fallbackTitle
  const description = catalog.messages.heroDescription ?? "Analyze your public Google Skills profile, review earned badges and check your Google Cloud Arcade reward tier."
...
export async function generateMetadata({ params }: LocalizedPageProps): Promise<Metadata> {
  ...
  return {
    title,
    description,
    alternates: { canonical, languages: getWebsiteLanguageAlternates() },
    openGraph: {
      title,
      description,
      url: canonical,
      locale: localeInfo.htmlLang.replace("-", "_"),
      alternateLocale: WEBSITE_LOCALES.filter((item) => item.code !== locale).map((item) => item.htmlLang.replace("-", "_")),
    },
