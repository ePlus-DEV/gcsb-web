...
test("widget is light by default without changing the website theme preference", () => {
  ...
  assert.match(defaultLightCss, /html:not\(\[data-widget-theme="dark"\]\) body/)
  assert.match(defaultLightCss, /background:\s*#ffffff !important/)
  assert.match(defaultLightCss, /html:not\(\[data-widget-theme="dark"\]\) \.arcade-embed-shell/)
  assert.match(defaultLightCss, /color-scheme:\s*light/)
