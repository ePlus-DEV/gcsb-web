...
test("browser client and calculator share one real history URL", () => {
  ...
  assert.match(component, /<ComposedChart/)
...
test("all tier lines are enabled by default and can be independently toggled", () => {
  ...
  assert.ok(ui.includes("const visible = DISPLAY_TIERS.filter((tier) => enabled.includes(tier.key))"))
  assert.match(ui, /<ComposedChart/)
...
test("slot-history trend chart has gradient series and navigable zoom without invented samples", () => {
  ...
  assert.match(ui, /series\.length >= 4/)
  assert.match(ui, /setZoom\(null\)/)
  assert.match(ui, /dataKey="ts"/)
  assert.match(ui, /ts: Math\.max\(cutoff, Date\.parse\(item\.at\)\)/)
...
test("all public slot history strings use a passed catalog and locale", () => {
  ...
  assert.match(calc, /loadWebsiteCatalog\(selectedLocale\)/)
